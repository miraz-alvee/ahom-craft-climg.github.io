...
export default function ResumePage() {
  ...
  const set = (f: string, v: string) => setData(d => ({ ...d, [f]: v }));
        ...
        <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1, background: "#fff", borderRadius: 16, border: "1px solid #f3f4f6", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", overflow: "hidden" }}>
            ...
            <div style={{ background: "linear-gradient(90deg, #3B82F6 40%, #6366F1 100%)", padding: "32px 36px 28px", position: "relative", overflow: "hidden" }}>
              ...
              {editing ? (
                <>
                  <Inp val={data.name} onChange={(v: string) => set("name", v)} style={{ fontSize: 26, fontWeight: 800, color: "#fff", marginBottom: 6, display: "block" }} wide />
                  <Inp val={data.title} onChange={(v: string) => set("title", v)} style={{ fontSize: 14, color: "rgba(255,255,255,0.85)", marginBottom: 18, display: "block" }} wide />
                </>
              ) : (
                <>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "5px 20px", fontSize: 12.5, color: "rgba(255,255,255,0.8)" }}>
                {([
                  ...
                  { f: "location", ico: "📍" },
                  { f: "linkedin", ico: "🔗" },
                ] as { f: keyof typeof DEFAULT; ico: string }[]).map(({ f, ico }, i) => (
                  <span key={f} style={{ display: "flex", alignItems: "center", gap: 5 }}>
                    {i > 0 && <span style={{ color: "rgba(255,255,255,0.3)", marginRight: 2 }}>•</span>}
                    <span>{ico}</span>
                    {editing
                      ? <Inp val={typeof data[f] === "string" ? data[f] as string : ""} onChange={(v: string) => set(f, v)} style={{ fontSize: 12.5, color: "rgba(255,255,255,0.9)", minWidth: 120 }} />
                      : typeof data[f] === "string" ? data[f] : ""}
                  </span>
                ))}
              </div>
            </div>
            ...
            <div style={{ padding: "28px 36px 36px" }}>
              ...
              <div style={{ marginBottom: 26 }}>
                ...
                {editing
                  ? <Textarea val={data.summary} onChange={(v: string) => set("summary", v)} rows={3} />
                ...
                {data.experience.map((exp) => (
                  <div key={exp.id} style={{ marginBottom: 22, position: "relative" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingRight: editing ? 90 : 0, marginBottom: 2 }}>
                      {editing ? <Inp val={exp.role} onChange={(v: string) => updExp({ id: exp.id, f: "role", v })} style={{ fontSize: 14, fontWeight: 700, color: "#111827" }} wide />
                        : <div style={{ fontSize: 14, fontWeight: 700, color: "#111827" }}>{exp.role}</div>}
                      {editing
                        ? <div style={{ display: "flex", gap: 4, flexShrink: 0, marginLeft: 8 }}>
                            <Inp val={exp.start} onChange={(v: string) => updExp({ id: exp.id, f: "start", v })} style={{ width: 75, fontSize: 12.5, color: "#6b7280", textAlign: "center" }} />
                            <span style={{ color: "#9ca3af", fontSize: 12 }}>–</span>
                            <Inp val={exp.end} onChange={(v: string) => updExp({ id: exp.id, f: "end", v })} style={{ width: 75, fontSize: 12.5, color: "#6b7280", textAlign: "center" }} />
                          </div>
                        : <div style={{ fontSize: 12.5, color: "#6b7280", flexShrink: 0 }}>{exp.start} – {exp.end}</div>}
                    </div>
                    {editing
                      ? <div style={{ display: "flex", gap: 5, marginBottom: 8 }}>
                          <Inp val={exp.company} onChange={(v: string) => updExp({ id: exp.id, f: "company", v })} style={{ fontSize: 13, color: "#4f63e7", fontWeight: 600 }} />
                          <span style={{ color: "#4f63e7" }}>·</span>
                          <Inp val={exp.location} onChange={(v: string) => updExp({ id: exp.id, f: "location", v })} style={{ fontSize: 13, color: "#4f63e7" }} />
                        </div>
                        ...
                        <li key={bi} style={{ display: "flex", alignItems: "flex-start", gap: 6, fontSize: 13.5, color: "#374151", lineHeight: 1.65, marginBottom: 4 }}>
                          ...
                          {editing
                            ? <div style={{ display: "flex", flex: 1, gap: 5 }}>
                                <Inp val={b} onChange={(v: string) => updBullet({ id: exp.id, i: bi, v })} wide style={{ fontSize: 13, color: "#374151" }} />
                ...
                {data.education.map((edu) => (
                  <div key={edu.id} style={{ marginBottom: 14, position: "relative" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", paddingRight: editing ? 90 : 0, alignItems: "center" }}>
                      {editing ? <Inp val={edu.degree} onChange={(v: string) => updEdu({ id: edu.id, f: "degree", v })} wide style={{ fontSize: 14, fontWeight: 700, color: "#111827" }} />
                        : <div style={{ fontSize: 14, fontWeight: 700, color: "#111827" }}>{edu.degree}</div>}
                      {editing
                        ? <div style={{ display: "flex", gap: 4, flexShrink: 0, marginLeft: 8 }}>
                            <Inp val={edu.start} onChange={(v: string) => updEdu({ id: edu.id, f: "start", v })} style={{ width: 55, fontSize: 12.5, color: "#6b7280", textAlign: "center" }} />
                            <span style={{ color: "#9ca3af", fontSize: 12 }}>–</span>
                            <Inp val={edu.end} onChange={(v: string) => updEdu({ id: edu.id, f: "end", v })} style={{ width: 55, fontSize: 12.5, color: "#6b7280", textAlign: "center" }} />
                          </div>
                        : <div style={{ fontSize: 12.5, color: "#6b7280" }}>{edu.start} – {edu.end}</div>}
                    </div>
                    {editing ? <Inp val={edu.school} onChange={(v: string) => updEdu({ id: edu.id, f: "school", v })} style={{ fontSize: 13, color: "#4f63e7", fontWeight: 600, marginTop: 3 }} wide />
