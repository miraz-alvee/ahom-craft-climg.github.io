    ...
    <div className="p-4 md:p-6 w-full h-full bg-[#f4f6fb]">
      <div className="max-w-7xl mx-auto">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Connect in real-time with students, employers, and team members.
            </p>
          </div>
        </div>
        ...
        <div className="h-188 max-h-[calc(100vh-12rem)] w-full overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-sm flex flex-col md:flex-row">
          <div className={`h-full w-full md:w-80 lg:w-96 shrink-0 ${activeRoom ? "hidden md:block" : "block"}`}>
            <ChatSidebar
          ...
          <div className={`h-full flex-1 min-w-0 ${activeRoom ? "block" : "hidden md:block"}`}>
            {activeRoom ? (
              <ChatWindow
                room={activeRoom}
